...
import { 
  ...
  LogOut, X, AlertCircle, Copy, AlertTriangle
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    getRedirectResult(auth)
      ...
      .catch((error) => setErrorDetails(error.code));
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked' || err.code === 'auth/popup-closed-by-user') {
        ...
        } catch (reErr) { setErrorDetails(reErr.code); }
      } else {
  ...
  if (!isConfigured) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-gray-50 p-6 text-center">
        ...
        <h1 className="text-2xl font-black mb-2">Firebase 未配置</h1>
        <p className="text-gray-500 max-w-sm">請編輯程式碼填入 firebaseConfig 資訊。</p>
      </div>
    );
  }

  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-white font-bold text-indigo-600">
      載入中...
    </div>
  );
        ...
        {errorDetails && (
          <div className="bg-red-50 p-4 rounded-2xl text-left border border-red-100">
            <div className="flex items-center gap-2 mb-1 text-red-700 font-bold text-xs">
              <AlertTriangle size={14} /> 登入受阻
            </div>
            <p className="text-red-500 text-[10px] font-mono mb-2">{errorDetails}</p>
            {errorDetails === 'auth/unauthorized-domain' && (
              <div className="flex items-center gap-2 bg-white p-2 rounded-lg border border-red-100">
                <code className="text-[9px] flex-1 truncate text-slate-500 font-bold">{window.location.hostname}</code>
                <button onClick={copyDomain} className="text-indigo-600">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col pb-20 md:pb-0 md:pl-64 font-sans antialiased text-slate-900">
      ...
      <nav className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-200 p-6 z-40">
        ...
        <div className="mt-auto pt-6 space-y-4">
          <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100 flex items-center gap-3">
            ...
            <div className="overflow-hidden"><p className="text-xs font-black text-gray-800 truncate">{user.displayName}</p></div>
          </div>
      ...
      {message && (
        <div className="fixed bottom-24 md:bottom-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-3 rounded-full shadow-2xl z-[100] flex items-center gap-2 animate-bounce font-bold text-sm">
...
function NavItem({ icon, label, active, onClick }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${active ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:bg-gray-50'}`}>
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input type="text" placeholder="搜尋..." className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-2xl shadow-sm outline-none focus:ring-2 focus:ring-indigo-500" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      <div className="grid gap-3 pb-10">
        {filtered.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 text-gray-400 font-bold">目前沒有資料</div>
        ) : (
          filtered.map(item => (
            <div key={item.id} className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden transition-all hover:shadow-md">
              <div className="p-5 flex items-center justify-between cursor-pointer" onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}>
                <div className="flex items-center gap-4">
                  <button onClick={(e) => toggleFav(e, item)}><Star className={`w-6 h-6 ${item.favorite ? 'fill-yellow-400 text-yellow-400' : 'text-gray-200'}`} /></button>
                  <div>
                    <h3 className="font-black text-xl text-gray-800">{item.word} <span className="ml-2 text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded uppercase">{item.pos}</span></h3>
                    <p className="text-gray-500 font-medium">{item.definition}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={(e) => remove(e, item.id)} className="p-2 text-gray-300 hover:text-red-500"><Trash2 size={18} /></button>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[2.5rem] shadow-xl space-y-6 border border-gray-50">
        ...
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase ml-2">英文例句 Example</label>
...
function Quiz({ vocab }) {
  ...
  useEffect(() => {
    if (vocab.length > 0) setShuffled([...vocab].sort(() => Math.random() - 0.5));
  }, [vocab]);

  if (shuffled.length === 0) return <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 text-gray-400 font-bold">單字庫目前沒有單字，無法開始練習。</div>;

  const current = shuffled[currentIdx];
  const handleNext = (e) => {
    e.stopPropagation();
    setIsFlipped(false);
    setTimeout(() => { setCurrentIdx((prev) => (prev + 1) % shuffled.length); }, 200);
  };

  return (
    <div className="max-w-md mx-auto space-y-10 py-6">
      <div className="relative h-[28rem] w-full cursor-pointer group" style={{ perspective: '1000px' }} onClick={() => setIsFlipped(!isFlipped)}>
        <div className={`relative w-full h-full transition-all duration-500 transform-gpu ${isFlipped ? 'rotate-y-180' : ''}`} style={{ transformStyle: 'preserve-3d' }}>
          ...
          <div className="absolute inset-0 bg-indigo-600 rounded-[3.5rem] shadow-xl flex flex-col items-center justify-center p-10 text-white rotate-y-180 backface-hidden">
            ...
            <button onClick={handleNext} className="mt-10 bg-white text-indigo-600 px-10 py-3 rounded-2xl font-black hover:bg-gray-50 transition-colors">下一個</button>
          </div>
        </div>
      </div>
